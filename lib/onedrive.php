...
class onedrive {
	...
	static function download_url($path) {
		$request = self::request($path, "?select=@microsoft.graph.downloadUrl");
		$resp = json_decode(fetch::get($request)->content, true);
		return !$resp ? false : $resp['@microsoft.graph.downloadUrl'];
	}
